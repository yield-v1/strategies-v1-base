...
describe.skip('XQiStrategyTest', async () => {
  ...
  before(async function () {
    ...
    await ISmartVault__factory.connect(data[0].address, _signer).changeDoHardWorkOnInvest(true);
    await ISmartVault__factory.connect(data[0].address, _signer).changeAlwaysInvest(true);
