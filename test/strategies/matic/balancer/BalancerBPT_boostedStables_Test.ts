...
describe.skip('BalancerBPT_boostedStables_Test', async () => {
  ...
  const underlying = BaseAddresses.BALANCER_bbamUSD;
  const poolId = BaseAddresses.BALANCER_bbamUSD_ID;
  const gauge = BaseAddresses.BALANCER_bbamUSD_GAUGE;
  const depositToken = BaseAddresses.bbamUSDC_TOKEN;
