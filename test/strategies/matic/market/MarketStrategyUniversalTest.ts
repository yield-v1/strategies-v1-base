...
describe.skip('Universal Market tests', async () => {
  ...
  const infos = readFileSync('scripts/utils/download/data/market.csv', 'utf8').split(/\r?\n/);
  const tokens = [BaseAddresses.USDC_TOKEN,
  ...
  infos.forEach(info => {
    ...
    if (+tvl < 1_00 || idx === 'idx' || !tokens.includes(tokenAdr.toLowerCase())) {
