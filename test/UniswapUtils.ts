...
export class UniswapUtils {
  ...
  public static async swapExactTokensForTokens(
    ...
    if (_router.toLowerCase() === BaseAddresses.FIREBIRD_ROUTER) {
      ...
      const fbFac = IFireBirdFactory__factory.connect(BaseAddresses.FIREBIRD_FACTORY, sender);
  ...
  private static isFeeToken(token: string) {
    return token.toLowerCase() === BaseAddresses.SPHERE_TOKEN.toLowerCase()
      || token.toLowerCase() === BaseAddresses.SPHEREV2_TOKEN.toLowerCase();
  }
  ...
  public static async addLiquidity(
    ...
    if (_factory.toLowerCase() === BaseAddresses.FIREBIRD_FACTORY.toLowerCase()) {
  ...
  public static async getTokenFromHolder(
    ...
    // if (token === BaseAddresses.WMATIC_TOKEN) {
  ...
  public static async getPairFromFactory(signer: SignerWithAddress, token0: string, token1: string, factory: string): Promise<string> {
    if (factory.toLowerCase() === BaseAddresses.FIREBIRD_FACTORY.toLowerCase()) {
